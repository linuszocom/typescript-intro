...
1. Öppna din terminal (Mac) eller PowerShell/Command Prompt (Windows).
2. Skriv följande kommando och tryck Enter:
   ```bash
   node -v
   npm -v
    ```
...
1. Skapa en ny mapp på datorn som heter `ts-intro` (eller valfritt namn).
2. Öppna denna mapp i **VS Code**.
3. Öppna terminalen inuti VS Code (`Terminal` -> `New Terminal`).
4. Initiera ett nytt projekt genom att skriva:
   ```bash
   npm init -y
   ```
...
1. Öppna filen `tsconfig.json` som skapades.
2. Ta bort allt innehåll i filen och klistra in nedanstående kod istället:
...
1. Skapa en ny mapp i ditt projekt som heter `src`.
2. Inuti `src`, skapa en fil som heter `index.ts`.
3. I roten av projektet (utanför src), skapa en fil som heter `index.html`.
...
1. Vi måste översätta TS till JS. Skriv i terminalen:
   ```bash
   npx tsc
   ```
...
1. Stanna kvar i webbläsarens utvecklarverktyg (F12) och klicka på fliken som heter **Sources**.
2. I menyn till vänster, leta upp mappen `dist` och klicka på `index.js`.
   *(Tips: Du kan också trycka `Ctrl + P` (PC) eller `Cmd + P` (Mac) och skriva "index" för att söka fram filen direkt).*
3. Klicka på **radnumret** bredvid raden där det står `return ...`. En färgad markering tänds. Du har nu satt en **Breakpoint**.
4. Ladda om sidan (tryck `F5` eller `Cmd + R`).
5. Sidan kommer sluta ladda och koden "fryser" vid din markering.
6. Håll muspekaren över variabeln `name` i koden. Ser du att det står "Alice"?
7. Tryck på **Play-knappen** (▶️) ute till höger i panelen för att låta koden köra klart.
